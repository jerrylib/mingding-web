...
class Request extends React.Component {
  render() {
    ...
    const { env, prefix, url, wds } = request.demo.setting
    let tip
    if (wds) {
      tip = (
        <span>
          本地请求代理转发，匹配前缀 <span className={style.highlight}>{prefix}</span> 转发到 <span className={style.highlight}>{url} </span>
        </span>
      )
    } else {
      tip = (
    ...
    return (
      <div className={style.request}>
        ...
        <div className={style['request-box']}>状态：{status}</div>
        <div className={style['request-box']}>结果：{JSON.stringify(data)}</div>
        <div className={style['request-box']}>
          提示：
          <ul>
            <li>
              当前请求环境：<span className={style.highlight}>{env}</span>
            </li>
            <li>请求地址：{tip}</li>
